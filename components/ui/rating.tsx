...
const ratingVariants = cva("transition-colors", {
  variants: {
    variant: {
      ...
      yellow: "fill-current text-amber-500",
    },
  },
  defaultVariants: {
    variant: "default",
...
function RatingItem({
  ...
  const icons = React.useMemo(() => {
    const emptyIcon = React.cloneElement(Icon, {
      size,
      className: cn("fill-muted-foreground/20 stroke-muted-foreground/10"),
      "aria-hidden": "true",
    });
...
function Rating({
  ...
  // Readonly: role="img" + aria-label only (aria-valuenow/min/max/text are illegal on img).
  // Interactive: role="radiogroup" (also must not use aria-value* — those belong on meter/slider).
  const accessibleLabel = readOnly
    ? `${value} av ${max} stjerner`
    : "Vurdering";
